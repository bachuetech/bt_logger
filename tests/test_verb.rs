...
mod logger_tests_params {
    ...
    fn test_get_all_msg_noparams_err(){
        ...
        println!("Test Get Msg {}: {:?}", LogLevel::ERROR, get_error!("test_level","ERROR MSG from {}","BT Logger"));          
    ...
    fn test_get_all_msg_wparams_fatal(){
        ...
        let patter_fatal = "BACHUETECH LOGGER.TEST .+ F test::logger_tests::test_level|>|FATAL MSG from BT Logger";
        build_logger_args("BACHUETECH", "LOGGER.TEST", &args);             
        println!("Test Get Msg {}: {:?}", LogLevel::FATAL, get_fatal!("test_level","FATAL MSG from {}","BT Logger"));         
        assert!(Regex::new(patter_fatal).unwrap().is_match( get_fatal!("test_level","FATAL MSG from {}","BT Logger").as_str() ) );
    }
